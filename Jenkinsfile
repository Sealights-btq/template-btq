...
pipeline {
  ...
  environment {
    ...
    SL_TOKEN = (sh(returnStdout: true, script:"aws secretsmanager get-secret-value --region eu-west-1 --secret-id 'btq/template' | jq -r '.SecretString' | jq -r '.template_token'" )).trim()
  }
    ...
    stage('update-btq') {
      steps {
        script {
          ...
          token: "${env.SL_TOKEN}",
          machine: "https://btq-template.sealights.co",
          app: "${params.APP_NAME}",
          branch: "${params.BRANCH}",
          test_env: "${IDENTIFIER}",
